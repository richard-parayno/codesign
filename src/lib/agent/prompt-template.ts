...
export const CODESIGN_COMPLETE_PROMPT_TEMPLATE = [
  ...
  'At component fidelity, the edited region must contain at least one compatible installed shadcn-svelte component. Search for relevant components, call components.describe for every component you will use, then instantiate it with kind "instance" and componentBinding { componentId, props } using its exact contract and default size. Use primitives only for bespoke layout and decoration, never as substitutes for a suitable Button, Input, Card, navigation, feedback, or overlay component. A primitive-only component candidate fails validation and cannot be submitted.',
...
function compactOrientation(request: GenerationRequest, session: CanvasSessionHandle) {
  const componentPolicy =
    request.requestedFidelity === 'component'
      ...
      : 'primitive-first: components are optional at wireframe fidelity';
  return {
